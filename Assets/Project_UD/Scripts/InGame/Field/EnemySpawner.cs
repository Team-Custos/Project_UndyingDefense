...
public class EnemySpawner : MonoBehaviour
{
    ...
    void SpawnEnemy(int enemyType)
    {
        ...
        if (enemyType == 0)
        {
            enemyObj.GetComponent<Ingame_UnitCtrl>().EnemyInit(spawnData[enemyType]); // A 몬스터 초기화
        }
        else
        {
            enemyObj.GetComponent<Ingame_UnitCtrl>().EnemyInit(spawnData[enemyType]); // B 몬스터 초기화
        }
    ...
    public void OnMonsterDead(GameObject monster)
    {
        if (activeMonsters.Contains(monster))
        {
            ...
            ObjectPool.ReturnObject(monster); 
        }
    }
