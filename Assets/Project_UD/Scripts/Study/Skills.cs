...
public class AttackSkill : Skills
{
    ...
    public override void Activate(Unit caster, Unit target)
    {
        ...
        if ((target.unitData.defenseType == DefenseType.cloth && attackType == AttackType.Crush)
            || (target.unitData.defenseType == DefenseType.leather && attackType == AttackType.Pierce)
            || (target.unitData.defenseType == DefenseType.metal && attackType == AttackType.Slash))
        {
            Damage *= 0.7f;
            crit *= 0.7f;
        }
